...
// Our parent block
describe('Setup For OUTBOUND sms Test', () => {
	before(async () => {
	 ...
	 */
	describe('OUTBOUND Endpoint Test' + OUTBOUND, () => {
		it('Throw validation error when required payload is missing', async () => {
			const response = await server.post(OUTBOUND)
		...
		it('Throw validation error when FROM parameter is not found', async () => {
			const response = await server.post(OUTBOUND)
				.send({ to: getSmsObject().to, from: '94857575', text: getSmsObject().text })
				.auth(authObject().username, authObject().password)
				.expect('Content-type', /json/)
				.expect(BAD_REQUEST);
			response.body.should.be.instanceOf(Object);
		...
		it('Throw error when FROM parameter is not found', async () => {
			const response = await server.post(OUTBOUND)
				.send({ to: getSmsObject().to, from: '94857575', text: getSmsObject().text })
				.auth(authObject().username, authObject().password)
				.expect('Content-type', /json/)
